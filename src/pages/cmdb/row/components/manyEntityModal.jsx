...
const ManyEntityForm = props => {
  ...
  const refs = fields.filter(val=>val.ref)
  ...
  const okHandle = async () => {
    ...
    let ref = refs.find(item=>item.id==fieldsValue.ref)

    handleAdd({ 
      ref: ref&&{id: ref.ref, name: ref.name},
      count: fieldsValue.record_count, 
      id: fieldsValue.id, 
      ...
      >
        ...
        >
          ...
          >
            ...
            >依赖新增</Select.Option>
            <Select.Option value="1">指定记录数新增</Select.Option>
          </Select>
        </FormItem>
        {method==0&&(
        <FormItem
        ...
        >
          <Select onChange={()=>{
            setSelectedRef(form.getFieldValue().ref)
          }}>
        {refs.map(item=><Select.Option key={item.id}>{item.name}</Select.Option>)}
          </Select>
        </FormItem>)}
        {method==1&&(
        <FormItem
          ...
          >
            {field.ref?(
              <Select 
                ...
                disabled={method==0&&field.id==selectedRef}
              >
